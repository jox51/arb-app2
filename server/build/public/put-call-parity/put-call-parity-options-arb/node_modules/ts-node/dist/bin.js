...
function phase4(payload) {
  ...
  if (isInChildProcess) require('./child/child-loader').lateBindHooks((0, index_1.createEsmHooks)(service));
  ...
  process.argv = [process.argv[1]].concat(executeEntrypoint ? [entryPointPath] : []).concat(restArgs.slice(executeEntrypoint ? 1 : 0));
...
function requireResolveNonCached(absoluteModuleSpecifier) {
  ...
  const {
    dir,
    base
  } = (0, path_1.parse)(absoluteModuleSpecifier);
  ...
  return req.resolve(relativeModuleSpecifier, {
    paths: [`${guaranteedNonexistentDirectoryPrefix}${guaranteedNonexistentDirectorySuffix++}`, ...(req.resolve.paths(relativeModuleSpecifier) || [])]
  });
}
/**
